...
public sealed class HologramClient(HttpClient httpClient, ILogger<HologramClient> logger) : IHologramClient
{
    ...
    private async Task<T> ReadResponseAsync<T>(HttpResponseMessage response, CancellationToken cancellationToken) where T : IHologramEnvelope
    {
        ...
        catch (JsonException ex)
        {
            ...
            throw new HologramUnavailableException(
                $"Hologram returned an unparseable response for {response.RequestMessage?.RequestUri}.", ex);
        }
        ...
        if (!response.IsSuccessStatusCode || payload is null || !payload.Success)
        {
            ...
            var detail = payload?.Error ?? $"Hologram returned status {(int)response.StatusCode}.";
            throw credentialRejected
                ? new HologramCredentialRejectedException(detail)
                : new HologramUnavailableException(detail);
        }
